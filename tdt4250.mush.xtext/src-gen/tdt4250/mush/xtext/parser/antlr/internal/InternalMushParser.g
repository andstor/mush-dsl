 ...
 */
parser grammar InternalMushParser;

options {
	tokenVocab=InternalMushLexer;
...
@after {
	...
	(
		...
		{
			$current = $this_CollectionSet_5.current;
			afterParserOrEnumRuleCall();
		}
		    |
		{
		...
		{
			$current = $this_ValueExchange_6.current;
			afterParserOrEnumRuleCall();
		}
	)
;
...
// Entry rule entryRuleCollectionSet
entryRuleCollectionSet returns [EObject current=null]:
	{ newCompositeNode(grammarAccess.getCollectionSetRule()); }
	iv_ruleCollectionSet=ruleCollectionSet
	{ $current=$iv_ruleCollectionSet.current; }
	EOF;

// Rule CollectionSet
ruleCollectionSet returns [EObject current=null]
@init {
	enterRule();
}
@after {
